...
const SubscriberApproval = () => {
  const {
    ...
    defaultValues: {
      subscriberApprovalForm: {
        subscriberList: [],
      },
    },
  });
  const useFormPropObj = {
    formName: "subscriberApprovalForm",
    register,
    getValues,
    setValue,
    watch,
  ...
  const handleDeleteClick = async (uID) => {
    console.log(uID);
    try {
      const userDocRef = doc(newDB, "subscribers_approval", uID);
  ...
  useEffect(() => {
    console.log(watch());
    const getSubscribersApprovalList = async () => {
      const newDBRef = collection(newDB, "subscribers_approval");
      const newSubscriberSnapshot = await getDocs(newDBRef);
      const newSubscribers = newSubscriberSnapshot.docs.map((doc) => {
        console.log(doc.id);
      ...
      // const filteredList = newSubscribers?.filter(x => typeof(x.email) !== 'undefined' || x.status !== 'PENDING')
      const filteredList = newSubscribers?.filter(
        (x) => typeof x?.email !== "undefined" || x?.email !== ""
      );
      console.log(filteredList);

      setValue("subscriberApprovalForm.subscriberList", filteredList);
    };

    getSubscribersApprovalList();
  }, []);
  return (
    <Grid
      container
        ...
        >
          ...
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {(getValues("subscriberApprovalForm.subscriberList") || [])?.map(
                  (user, userInd) => (
