...
const DynamicAccordion = ({ dataObj, useFormPropObj, uniqueKey, arrName}) => {
  ...
  const appendObjToArr = () => {
    ...
    useFormPropObj.append(dataObj?.appendObj ?? { title: '', description: '', imgUrl: '' })

    let tempArr = useFormPropObj.getValues(`${useFormPropObj.formName}.${uniqueKey}.${arrName ?? 'imgs_arr'}`) || [];
    tempArr.push(dataObj?.appendObj ?? { title: '', description: '', imgUrl: '' });
